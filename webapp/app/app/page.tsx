...
function VaultPanel({
  ...
  // Simplified local encryption for demo (in production use SubtleCrypto)
  const encrypt = (txt: string, pass: string) => btoa(txt + ':' + pass)
  const decrypt = (blob: string, pass: string) => {
    try {
      const decoded = atob(blob)
      const [txt, p] = decoded.split(':')
      return p === pass ? txt : null
    } catch { return null }
  }

  const loadVault = () => {
    const saved = localStorage.getItem(`vault_${userId}`)
    if (saved) {
      const parsed = JSON.parse(saved)
      const decrypted = parsed.map((item: any) => ({
        ...item,
        value: decrypt(item.value, password) || '●●●●●●'
      }))
      setItems(decrypted)
      setUnlocked(true)
    } else {
  ...
  const saveItem = () => {
    ...
    const updated = [
      ...items,
      { id: Date.now().toString(), label: newItem.label, value: newItem.value, type: 'text' as const }
    ]
    const encrypted = updated.map(item => ({
      ...item,
      value: encrypt(item.value, password)
    }))
    localStorage.setItem(`vault_${userId}`, JSON.stringify(encrypted))
    setItems(updated)
  ...
  return (
    <motion.div
      ...
      className="fixed right-0 top-0 bottom-0 w-80 z-50 shadow-2xl p-6 flex flex-col"
      style={{ background: '#F7F6F3', borderLeft: '1px solid #E5E5E0' }}
    >
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Lock size={20} className="text-emerald-600" /> Secure Vault
        </h2>
      ...
      {!unlocked ? (
        <div className="flex-1 flex flex-col justify-center gap-4">
          <p className="text-xs text-gray-500 text-center">
            Enter your private Master Password to unlock your data.
          </p>
          <input
            type="password"
            value={password}
            ...
            className="w-full px-4 py-2.5 rounded-xl text-sm border border-gray-200 outline-none focus:border-emerald-500"
          />
          <button
            onClick={loadVault}
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 mb-6">
            <p className="text-[10px] font-bold text-emerald-800 uppercase tracking-widest mb-1">Privacy Status</p>
            <p className="text-[11px] text-emerald-700 leading-relaxed">
              Zero-Knowledge Mode Active. Your data is encrypted locally and never seen by our servers.
            </p>
          </div>

          {items.map((item) => (
            <div key={item.id} className="p-3 bg-white rounded-xl border border-gray-200 group">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-gray-400 uppercase">{item.label}</span>
                <button className="opacity-0 group-hover:opacity-100 text-red-500 text-[10px] font-bold">Remove</button>
              </div>
          ...
          <div className="mt-8 pt-8 border-t border-gray-200">
            <p className="text-xs font-bold mb-3">Add New Document/Detail</p>
            <input
              type="text"
              placeholder="Label (e.g. Bank A/C)"
              value={newItem.label}
              onChange={e => setNewItem({ ...newItem, label: e.target.value })}
              className="w-full mb-2 px-3 py-2 text-xs rounded-lg border border-gray-200"
            />
            <input
              type="text"
              placeholder="Value"
      ...
      <div className="mt-auto pt-6 text-[10px] text-gray-400 text-center">
        <ShieldCheck size={12} className="inline mr-1" /> DPDP Act Compliant Storage
      </div>
    </motion.div>
  )
}
...
function BrowserPreview({
  ...
  const isLoading = frame === 'loading'

  /* minimized pill with pulsing effect */
  if (minimized) {
    return (
      <motion.button
        layout
        ...
        onClick={onMaximize}
        className="fixed bottom-24 right-4 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center group"
        style={{ background: '#0C0C0C', color: '#F7F6F3' }}
      ...
      >
        ...
        <Monitor size={20} className="relative z-10 group-hover:scale-110 transition-transform" />
        ...
        <svg className="absolute inset-0 w-14 h-14 -rotate-90" viewBox="0 0 56 56">
          ...
          <circle
            ...
            strokeDasharray={`${150.7 * progress} 150.7`}
            strokeLinecap="round"
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-24 right-4 z-50 w-[400px] rounded-2xl shadow-2xl overflow-hidden"
      style={{ background: 'white', border: '1px solid #E5E5E0' }}
    >
      {/* header */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold tracking-tight flex items-center gap-1.5">
            <Monitor size={14} /> LIVE PORTAL FILLING</span></div>
        <div className="flex items-center gap-2">
          <button 
            onClick={onMinimize} 
      ...
      <div className="aspect-[4/3] bg-[#1A1A1A] relative group">
        {isLoading ? (
          <div className="w-full h-full flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-[10px] text-white/50 font-bold uppercase tracking-widest">Connecting to Portal...</p>
          </div>
        ) : (
          <img
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center gap-3 bg-black/60 backdrop-blur-md px-3 py-2 rounded-xl border border-white/10">
          <div className="flex-1">
            <div className="text-[10px] text-white/50 uppercase font-bold tracking-wider mb-0.5">Current Action</div>
            <div className="text-xs text-white font-medium truncate">{step || 'Navigating...'}</div>
          </div>
          <button
            onClick={onStop}
            className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-[10px] font-bold rounded-lg transition-colors flex items-center gap-1.5"
          >
            <X size={10} /> STOP & CORRECT
          </button>
        </div>
      </div>
...
export default function AppPage() {
  ...
  const [liveTranscript, setLiveTranscript] = useState('') // realtime interim transcript
  ...
  /* ── Stop Browser Filling ────────────────────────────────── */
  const stopBrowserFilling = async () => {
    try {
      ...
      addMsg('system', '🛑 Form filling stopped by user. You can now correct the data.')
    } catch (err) {
  ...
  const callBackend = useCallback(
    async (text: string, phoneOverride?: string) => {
      ...
      try {
        ...
        // Intelligent Auto-play: Trigger if user asked to speak/read
        const speakTriggers = ['speak', 'sunaiye', 'sunao', 'bolo', 'read', 'audio', 'voice', 'बताना', 'सुनाओ']
        const userAskedToSpeak = speakTriggers.some(t => text.toLowerCase().includes(t))
        if (userAskedToSpeak && data.response) {
          setTimeout(() => playVoice(newMsgId, data.response), 500)
        }
      } catch {
        addMsg(
          'system',
  ...
  /* ── voice recording (realtime WebSocket) ── */
  const realtimeWs = useRef<WebSocket | null>(null)
  const audioContextRef = useRef<AudioContext | null>(null)
  const processorRef = useRef<ScriptProcessorNode | null>(null)
  const streamRef = useRef<MediaStream | null>(null)
  // Stable ref so ws callbacks can call stopRealtimeVoice without stale closures
  const stopRealtimeVoiceRef = useRef<() => void>(() => {})

  const stopRealtimeVoice = useCallback(() => {
    if (realtimeWs.current) {
      try { realtimeWs.current.send(JSON.stringify({ type: 'stop' })) } catch {}
      realtimeWs.current.close()
  ...
  // Keep the ref always pointing at the latest stop function
  useEffect(() => {
  ...
  const startRealtimeVoice = useCallback(async () => {
    ...
    try {
      ...
      // Determine host from window location but target the backend port 8000 directly
      const wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
      const wsUrl = `${wsProtocol}//${window.location.hostname}:8000/api/voice/realtime`
      const ws = new WebSocket(wsUrl)
      realtimeWs.current = ws

      ws.onopen = () => {
        ws.send(JSON.stringify({ type: 'start', language: langOverride || 'hi' }))
        setRecording(true)
        setLiveTranscript('')
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'error') {
            console.error('[Realtime STT] Server error:', data.message)
            setLiveTranscript('')
      ...
      ws.onerror = (err) => {
        console.error('[Realtime STT] WebSocket error:', err)
        stopRealtimeVoiceRef.current()
      }
      ...
      console.error('[Realtime STT] getUserMedia failed:', err)
      alert('Microphone access denied or hardware not found. Please check browser permissions.')
      setRecording(false)
    }
  }, [voiceSupported, handleSend, langOverride])
  /* ── legacy voice recording (fallback) ─────────────────────── */
  /* ── voice recording ───────────────────────────────────────── */
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: '#F7F6F3' }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-[10px] px-2.5 py-1 rounded-full flex items-center gap-1 font-bold"
            style={{ border: '1px solid #E5E5E0', color: '#059669', background: '#ecfdf5' }}
          >
            <Database size={11} /> VAULT
          </button>
          ...
          {/* Live Portal Button (New) */}
          {(browserFrame || status === 'loading') && (
            <button
              onClick={() => {
                setBrowserMinimized(false)
                if (!browserFrame) {
                  // Fallback to show the panel if it's not open
                  setBrowserFrame('loading')
                }
              }}
              className="text-[10px] px-2.5 py-1 rounded-full flex items-center gap-1 font-bold animate-pulse"
              style={{ border: '1px solid #22c55e', color: '#22c55e', background: '#f0fdf4' }}
            >
              <Monitor size={11} /> LIVE PORTAL
            </button>
          )}
              ...
              >
                ...
                {m.role === 'assistant' && (
                  <button
                    ...
                    className={`mt-2 flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold p-1 px-2 rounded-md transition-all ${
                      playingId === m.id 
                        ? 'bg-[#0C0C0C] text-white animate-pulse' 
                        : 'bg-black/5 hover:bg-black/10 text-[#6B6B6B]'
                    }`}
                  >
                    <Volume2 size={12} />
                    {playingId === m.id ? 'Listening...' : 'Listen'}
                  </button>
                )}
      ...
      {/* ── Live transcript overlay (realtime STT) ───────────────── */}
      {liveTranscript && (
        <div
          className="flex-shrink-0 px-4 py-2 flex items-start gap-2"
          style={{ background: '#eff6ff', color: '#1d4ed8', borderTop: '1px solid #bfdbfe' }}
        >
          <span className="inline-block w-2 h-2 rounded-full bg-blue-500 animate-pulse mt-1 flex-shrink-0" />
      ...
      >
        ...
        {voiceSupported && (
          <button
            onClick={recording ? stopRealtimeVoice : startRealtimeVoice}
